...
Please note that to vote on mainnet, you need to already have one of the following setups:
- the baking infrastructure on mainnet;
- a connected ledger device with Tezos Wallet app running;
...
baking on which needs to be set up already.
...
class Setup(Setup):
    ...
    def collect_baking_info(self):
        if self.check_baking_service():
            ...
            self.config["node_rpc_addr"] = "http://localhost:8732"
